...
object SPoSCalc extends ScorexLogging {
  ...
  val SlotGap = if (AddressScheme.current.chainId == ';'.toByte) 4 else 1

  // update plan: 15 slots -> 2 vsys coins, 30 slots -> 1 vsys coins, 60 slots -> 0.5 vsys coins
  val BaseReward = 50000000L
